...
var Erknugs = exports.Erknugs = function (opts) {
	...
	erk.on('hook::ready', function(){
	  ...
	  erk.on('*::sentry_response', function (data) {
	    data = JSON.parse(data.body)

	    erk.say({"to":"#dev", "msg": "[exception] " + data.project_name + ": " + data.message + " -- https://app.getsentry.com/" + data.project + "/group/" + data.id + "/" })
	  })
	});
	...
	webhookServer.on('hook::ready', function () {
	  webhookServer.on('webhook::request', function (data) {
	    ...
	    else if (data.url == '/sentry') {
	      webhookServer.emit('sentry_response',data)
	    }
	  })
	})
